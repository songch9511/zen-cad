...
- dependency-free validator, generator, inspector, adapter 제공;
...
실제 CAD 생성, STEP export, geometry measurement는 active CAD harness에서 수행해야 합니다. Zen CAD는 그 전에 spec, scene, source, inspection, proceed, detail handoff의 계약을 정리합니다.
...
6. schema와 registry 기준으로 contract package를 검증합니다.
7. 표면 품질보다 positioning을 우선하는 kernel-neutral layout proxy scene을 생성합니다.
8. layout proxy가 locked facts, parts, relationships, interface datums를 보존하는지 검사합니다.
9. CAD source adapter로 source intent와 manifest를 생성합니다.
10. source-level carry-through를 검사합니다.
11. 사용자 proceed review용 package를 만듭니다.
12. proceed 후 locked facts를 유지하는 detail CAD handoff packet을 생성합니다.
...
| Surface | 역할 |
| --- | --- |
| [`schemas/`](schemas) | CAD spec, layout contract, interface signature, inspection report, handoff packet, layout proxy scene, source manifest, proceed package JSON Schemas. |
...
python3 tools/validate_contract.py --package <contract-package>

python3 tools/generate_layout_proxy.py --package <contract-package> --out <layout-dir>
python3 tools/inspect_layout_proxy.py --package <contract-package> --scene <layout-dir>/layout_proxy.scene.json --out <layout-report.json>

python3 tools/export_cad_source.py --package <contract-package> --scene <layout-dir>/layout_proxy.scene.json --out <source-dir>
python3 tools/inspect_cad_source.py --scene <layout-dir>/layout_proxy.scene.json --manifest <source-dir>/cad_source_manifest.json --source <source-dir>/layout_proxy_build123d.py --out <source-report.json>

python3 tools/package_proceed_gate.py --package <contract-package> --artifact <layout-dir>/layout_proxy.scene.json --artifact <source-dir>/layout_proxy_build123d.py --report <layout-report.json> --report <source-report.json> --out <proceed-gate.json>
