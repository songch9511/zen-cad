...
def write_cobra_workspace_context(root: Path, target_dir: Path, skill_name: str) -> None:
    context = target_dir / 'ZEN_CAD_WORKSPACE.md'
    context.write_text(
        '\n'.join(
            [
                '# Zen CAD Workspace Binding',
                '',
                f'Skill: /{skill_name}',
                f'Repository root: {root}',
                '',
                'This file is generated by `./zen-cad init --with-cobra`.',
                '',
                ...
                'Create prompt-first milestones from that root with:',
                '',
                '```bash',
                'python3 scripts/new_milestone.py --request "<goal>"',
                '```',
                '',
                'Run structure checks from that same root before reporting first artifact',
                'paths, and reserve completion gates for final/release claims:',
                '',
                '```bash',
